...
SELECT nombre_comun, agua_requerida 
FROM Cultivos 
WHERE agua_requerida > '500mm';
...
SELECT * 
FROM Parcelas 
WHERE id_riego = 1 AND area > 5;


-- 4.	¿Qué insumos son semillas o fertilizantes?

SELECT nombre, id_tipo_insumo 
FROM Insumos 
WHERE id_tipo_insumo IN (1, 3);


-- 5.	¿Cuáles son las cosechas realizadas entre el 1 de enero y el 31 de marzo de 2024?

SELECT * 
FROM Cosechas 
WHERE fecha_ini BETWEEN '2024-01-01' AND '2024-03-31';


-- 6.	¿Qué fincas están ubicadas en las regiones Valle Central, Zona Norte o Pacífico Sur?

SELECT * 
FROM Fincas 
WHERE region IN ('Valle Central','Zona Norte','Pacífico Sur')
...
SELECT id_parcela, codigo, IFNULL(HistorialUso, 'Sin registro') AS Estado
FROM Parcelas 
WHERE HistorialUso IS NULL OR HistorialUso = '';
...
SELECT CultivoID, ROUND(AVG(RendimientoEsperado), 2) AS Rendimiento_Promedio
FROM Siembras 
GROUP BY CultivoID;
...
-- 1. Utiliza INNER JOIN con WHERE para encontrar todas las parcelas de fincas ubicadas en una región específica y que tengan sistema de riego por goteo.
SELECT 
    f.nombre AS nombre_finca,f.region,p.id_finca,p.codigo AS codigo_parcela,p.id_riego,r.nombre AS nombre_riego
FROM fincas f
INNER JOIN parcelas p ON p.id_finca = f.id_finca
INNER JOIN sistemas_riego r ON p.id_riego = r.id_riego
WHERE f.region = 'Pacífico Sur' 
  AND r.nombre = 'Goteo';
--  ------------------------------------------------------------------------------

-- 2. Aplica LEFT JOIN con ORDER BY para listar todas las fincas y sus parcelas (si las tienen) ordenadas por la extensión total de la finca de mayor a menor.
SELECT 
    f.nombre AS nombre_finca,f.region,f.extension,p.codigo AS codigo_parcela
FROM fincas f
LEFT JOIN parcelas p ON p.id_finca = f.id_finca
ORDER BY f.extension DESC;
--  ------------------------------------------------------------------------------

-- 3. Usa RIGHT JOIN con GROUP BY y HAVING para encontrar cultivos que han sido sembrados en más de 5 parcelas diferentes y calcular su rendimiento promedio.
SELECT 
    c.nombre_comun, 
    COUNT(DISTINCT s.ParcelaID) AS total_parcelas, 
    AVG(s.RendimientoEsperado) AS rendimiento_promedio
FROM cultivos c
RIGHT JOIN siembras s ON c.id_cultivo = s.CultivoID
GROUP BY c.nombre_comun
HAVING COUNT(DISTINCT s.ParcelaID) > 5;
--  ------------------------------------------------------------------------------

-- 4. Implementa INNER JOIN múltiple con BETWEEN para listar cosechas realizadas entre dos fechas específicas junto con los datos de la parcela y el cultivo.
SELECT 
    s.SiembraID,p.codigo AS codigo_parcela,c.nombre_comun AS cultivo,s.FechaSiembra,s.FechaEstCosecha,s.RendimientoEsperado
FROM siembras s
INNER JOIN parcelas p ON s.ParcelaID = p.id_parcela
INNER JOIN cultivos c ON s.CultivoID = c.id_cultivo
WHERE s.FechaEstCosecha BETWEEN '2024-06-01' AND '2024-12-31'
ORDER BY s.FechaEstCosecha ASC;
--  ------------------------------------------------------------------------------

-- 5. Combina LEFT JOIN con IS NULL para identificar fincas que no tienen parcelas registradas en el sistema.
SELECT 
    f.id_finca, f.nombre AS nombre_finca, p.id_parcela
FROM fincas f
LEFT JOIN parcelas p ON f.id_finca = p.id_finca
WHERE p.id_parcela IS NULL;
--  ------------------------------------------------------------------------------

-- 6. Utiliza INNER JOIN con IN para encontrar parcelas que han sido sembradas con cualquiera de tres cultivos específicos en la última temporada.
SELECT 
    p.id_parcela, 
    p.codigo, 
    c.nombre_comun, 
    s.FechaSiembra
FROM parcelas p
INNER JOIN siembras s ON p.id_parcela = s.ParcelaID
INNER JOIN cultivos c ON s.CultivoID = c.id_cultivo
WHERE c.nombre_comun IN ('Maíz', 'Cebolla', 'Zanahoria')
  AND s.FechaSiembra >= '2024-01-01';
--  ------------------------------------------------------------------------------

-- 7. Aplica JOIN con función de agregación SUM y GROUP BY para calcular la cantidad total de insumos aplicados por tipo en cada finca.
SELECT 
    f.nombre AS nombre_finca,ti.nombre AS categoria_insumo,i.nombre AS producto,SUM(i.stock) AS cantidad_total,SUM(i.stock * i.precio) AS valor_monetario
FROM fincas f
INNER JOIN insumos i ON f.id_finca = i.id_almacen
INNER JOIN tipos_insumo ti ON i.id_tipo_insumo = ti.id_tipo_insumo
GROUP BY f.nombre, ti.nombre, i.nombre
ORDER BY f.nombre ASC, ti.nombre ASC;
--  ------------------------------------------------------------------------------

-- 8. Usa INNER JOIN con LIKE para encontrar todas las cosechas de cultivos cuyo nombre contiene cierta palabra, incluyendo información de la parcela.
SELECT 
    s.SiembraID,c.nombre_comun AS cultivo,p.codigo AS codigo_parcela,s.FechaEstCosecha,s.RendimientoEsperado
FROM siembras s
INNER JOIN cultivos c ON s.CultivoID = c.id_cultivo
INNER JOIN parcelas p ON s.ParcelaID = p.id_parcela
WHERE c.nombre_comun LIKE '%Maíz%'; 
--  ------------------------------------------------------------------------------

-- 9. Implementa JOIN múltiple con subconsulta para identificar las parcelas con rendimiento superior al promedio para cada tipo de cultivo.
SELECT 
    p.codigo AS codigo_parcela,c.nombre_comun AS cultivo,s.RendimientoEsperado,promedios.rendimiento_medio_cultivo -- Tabla Temporal donde guarda el resultado.
FROM siembras s
INNER JOIN parcelas p ON s.ParcelaID = p.id_parcela
INNER JOIN cultivos c ON s.CultivoID = c.id_cultivo
INNER JOIN (
    -- Subconsulta para calcular el promedio por cada tipo de cultivo
    SELECT CultivoID, AVG(RendimientoEsperado) AS rendimiento_medio_cultivo
    FROM siembras
    GROUP BY CultivoID
) AS promedios ON s.CultivoID = promedios.CultivoID
WHERE s.RendimientoEsperado > promedios.rendimiento_medio_cultivo;
--  ------------------------------------------------------------------------------

-- 10. Combina LEFT JOIN con función de fecha para listar los análisis de suelos realizados en los últimos 6 meses junto 
-- con los datos de las parcelas, incluso si no tienen análisis.
SELECT 
    p.codigo AS codigo_parcela, p.area,a.FechaMuestreo,a.pH,a.Recomendaciones
FROM parcelas p
LEFT JOIN analisis_suelo a ON p.id_parcela = a.ParcelaID 
    AND a.FechaMuestreo >= DATE_SUB(NOW(), INTERVAL 16 MONTH)
ORDER BY a.FechaMuestreo DESC;
--  ------------------------------------------------------------------------------
